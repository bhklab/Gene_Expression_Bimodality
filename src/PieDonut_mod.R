...
PieDonut_mod <- function(data, mapping, piesLevels = NULL,start = getOption("PieDonut.start",
                                                                            0), addPieLabel = TRUE, addDonutLabel = TRUE, showRatioDonut = TRUE, showlabelsDonut = TRUE,
                         ...
                         family = getOption("PieDonut.family", "")) 
{
  ...
  if (!is.null(donuts)) {
    ...
    if (!is.null(count)) {
      ...
      tmp <- table(data[[donuts]], data[[pies]])
      if(!is.null(piesLevels))
        tmp <- tmp[,piesLevels]
      df3 = data.frame(tmp, 
    ...
    if (showRatioDonut) {
      if(showlabelsDonut){
        ...
        if (max(nchar(levels(df3$label))) <= 2) 
          df3$label = paste0(df3$ratio)
        else df3$label = paste0(df3$ratio)
      }
    }
